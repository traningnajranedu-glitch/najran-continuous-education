...
async function synthesize(ssml) {
  return fetch(`https://${AZURE_SPEECH_REGION}.tts.speech.microsoft.com/cognitiveservices/v1`, { method: "POST", headers: { "Ocp-Apim-Subscription-Key": AZURE_SPEECH_KEY, "X-Microsoft-OutputFormat": "audio-24khz-160kbitrate-mono-mp3", "Content-Type": "application/ssml+xml" }, body: ssml, cache: "no-store" });
}
...
export async function POST(request) {
  try {
    ...
    if (requestStatus?.protected) {
      if (!requestStatus.text.includes("لا يمكن عرض حالة الطلب")) {
    ...
    if (requestStatus?.text && requestStatus.text.includes("لا يمكن عرض حالة الطلب")) enrichedMessage += `\n\nتنبيه أمني: ${requestStatus.text}`;
