...
export async function GET(request) {
  try {
    ...
    const params = new URLSearchParams({ select: "id,school_id,report_type,report_date,title,summary,report_data,source_file_name,status,reported_by,created_at", school_id: `eq.${member.school_id}`, status: "eq.approved", order: "report_date.desc,created_at.desc", limit: "200" });
...
export async function POST(request) {
  try {
    ...
    const allowed = ["students", "teachers", "environment", "activities"];
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/school_periodic_reports`, {
      ...
      body: JSON.stringify({ school_id: member.school_id, report_type: reportType, report_date: reportDate, title: title || null, summary: summary || null, report_data: reportData, source_file_name: fileName || null, status: "approved", reported_by: member.user_id || null }),
      cache: "no-store",
    });
    const data = await readJson(response);
    if (!response.ok) return NextResponse.json({ error: data?.message || "تعذر حفظ التقرير." }, { status: response.status || 500 });
    return NextResponse.json({ ok: true, school: member.school_name, report: Array.isArray(data) ? data[0] : data });
  } catch (error) {
