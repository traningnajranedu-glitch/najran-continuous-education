...
function teacherStructureScore(rows) {
  const keys = new Set(rows.slice(0, 100).flatMap((r) => Object.keys(r || {})));
  const hasName = [...keys].some((k) => ["الاسم", "اسم المعلم", "اسم المعلم/المعلمه", "اسم المعلم/المعلمة", "اسم"].some((x) => k === norm(x) || k.includes(norm(x))));
  const hasCivil = [...keys].some((k) => ["سجل المدني", "السجل المدني", "رقم الهوية", "الهوية"].some((x) => k === norm(x) || k.includes(norm(x))));
  const hasSchool = [...keys].some((k) => k.includes("مدرسة الترشيح"));
  const hasStatus = [...keys].some((k) => k.includes("حالة الترشيح"));
  let score = 0;
  if (hasName) score += 5;
  if (hasCivil) score += 6;
  if (hasSchool) score += 6;
  if (hasStatus) score += 4;
  return { score, hasName, hasCivil, hasSchool, hasStatus };
}
...
function detectFieldIssues(rows, detectedType) {
  ...
  if (detectedType === "teachers") {
    ...
    const statusKey = keys.find((k) => k.includes("حالة الترشيح") || k === "الحاله" || k === "الحالة");
...
export function classifyReport({ title = "", summary = "", fileName = "", reportType = "", rows = [] }) {
  ...
  const structure = teacherStructureScore(cleanRows);

  const scores = Object.entries(RULES)
    .map(([key, rule]) => ({ key, label: rule.label, score: similarityScore(corpus, rule) + (key === "teachers" ? structure.score : 0) }))
    .sort((a, b) => b.score - a.score);

  let best = scores[0] || { key: "unknown", label: "غير محدد", score: 0 };
  const second = scores[1] || { score: 0 };
  if (structure.score >= 11) best = { key: "teachers", label: RULES.teachers.label, score: Math.max(best.score, 20) };

  const confidence = structure.score >= 11 && best.key === "teachers"
    ? 0.95
    : Math.min(0.99, Math.max(0.35, best.score / (best.score + second.score + 1)));
  ...
  const validationStatus = mismatch ? "review" : issues.some((x) => !x.includes("لن يتم احتساب")) ? "review" : "ok";
...
function buildMetrics(rows, type) {
  if (type === "requests") {
    const statuses = {};
    const statusColumnFound = rows.length > 0 && Object.keys(rows[0]).some((k) => k === "الحاله" || k === "الحالة" || k.includes("حالة الطلب"));
    for (const row of rows) {
  ...
  if (type === "teachers") {
    ...
    const statusColumnFound = rows.length > 0 && Object.keys(rows[0]).some((k) => k.includes("حالة الترشيح") || k === "الحاله" || k === "الحالة");
    for (const row of rows) {
