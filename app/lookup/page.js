    ...
    <section className={`card lookup-card ${person ? `status-${type}` : ""}`}>
      ...
      {id && person && <div className={`result ${type}`}>
        <div className="result-title">
          <span className="status-icon">{type === "selected" ? "✓" : type === "not-selected" ? "!" : "•"}</span>
          <div><h2>{type === "selected" && "تم العثور على الترشيح"}{type === "not-selected" && "لم يتم الترشيح"}{type === "pending" && "الطلب تحت الإجراء"}{type === "apology" && "حالة الترشيح: معتذر"}</h2><span className="status-caption">حالة الترشيح</span></div>
        </div>
        <div className="print-content">
          <div className="print-header"><img src="/moe-logo.jpg" alt="شعار وزارة التعليم" className="print-logo" /><div className="print-org">الإدارة العامة للتعليم بمنطقة نجران</div><div className="print-title">نتيجة الاستعلام عن الترشيح</div></div>
          <div className="details"><p><span>الاسم</span><b>{person.name}</b></p><p><span>المدرسة</span><b>{person.school || "—"}</b></p><p><span>التخصص</span><b>{person.specialty || "—"}</b></p><p><span>الحالة</span><b className={`status-text ${type}`}>{person.status || "—"}</b></p></div>
          <div className="message">{message}</div><div className="print-date">تاريخ الاستعلام: {new Date().toLocaleDateString("ar-SA")}</div>
        </div>
        {type !== "not-selected" && <div className="print-actions"><PrintButton /></div>}
      </div>}

      {id && !person && <div className="error"><b>لم يتم العثور على بيانات.</b><br/>تأكد من رقم السجل المدني، أو راجع إدارة التعليم إذا كنت تتوقع وجود ترشيح.{error && <small className="technical-error">تعذر الاتصال بقاعدة البيانات حاليًا.</small>}</div>}
      <Link href="/" className="back">← العودة للمساعد التعليمي الذكي</Link>
    </section>
  </main>;
}
