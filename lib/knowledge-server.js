...
function sanitizePublicReport(report) {
  const rows = Array.isArray(report.public_report_data) ? report.public_report_data : [];
  return rows.map((row) => ({
    "الاسم": row?.["الاسم"] || "",
    "مدرسة الترشيح": row?.["مدرسة الترشيح"] || "",
  })).filter((row) => row["الاسم"] || row["مدرسة الترشيح"]);
}
...
export async function searchKnowledgeFromSupabase(query) {
  ...
  const [{ data: knowledgeData, error: knowledgeError }, { data: reportData, error: reportError }] = await Promise.all([
    client.from("knowledge_base")
      ...
      .limit(200),
    client.from("public_school_reports")
      .select("id,report_type,report_date,title,summary,public_report_data,source_file_name,status,created_at")
      .eq("status", "approved")
  ...
  const reportItems = !reportError && Array.isArray(reportData) ? reportData.map((report) => {
    const publicRows = sanitizePublicReport(report);
    return {
      ...
      content: [
        ...
        report.summary || "",
        publicRows.length ? `عدد السجلات المنشورة: ${publicRows.length}.` : "",
        publicRows.length ? `بيانات التقرير المنشورة: ${JSON.stringify(publicRows)}` : "",
      ].filter(Boolean).join("\n"),
      keywords: ["تقرير دوري", "تقارير مدرسية", reportLabels[report.report_type] || report.report_type, report.source_file_name || ""].filter(Boolean),
      sourceUrl: null,
      documentUrl: null,
