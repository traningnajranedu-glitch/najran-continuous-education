...
export default function SchoolsPortal() {
  ...
  async function login(e) {
    e.preventDefault();
    if (!supabase) return setMsg("إعدادات Supabase للواجهة غير مكتملة.");
    setLoggingIn(true); setMsg("");
    try {
      const { data, error } = await supabase.auth.signInWithPassword({ email: email.trim(), password });
      if (error) throw error;
      setSession(data.session);
      const response = await fetch("/api/schools/me", { headers: { Authorization: `Bearer ${data.session.access_token}` } });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "تعذر التحقق من صلاحيات المدرسة.");
  ...
  async function logout() {
    ...
    setSession(null); setSchool(null); setRows([]); setFileName(""); setMsg("");
  }
    ...
    return (
      <main className="shell" dir="rtl">
        <section className="card" style={{ maxWidth: 520 }}>
          ...
          <div style={{ marginTop: 18, fontSize: 13, lineHeight: 1.8, color: "var(--muted)" }}>تُمنح صلاحية المدرسة من قبل المشرف، ولا يمكن إنشاء عضوية مدرسة من هذه الصفحة.</div>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="shell" dir="rtl">
      <section className="card" style={{ maxWidth: 1120 }}>
        <div className="page-heading">
          <div><div className="section-label">بوابة المدارس</div><h1>رفع تقرير حالة الطلبات</h1><p>المدرسة: <strong>{school.name}</strong></p></div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div className="heading-mark">Excel</div><button className="secondary" type="button" onClick={logout}>خروج</button></div>
        </div>

        <div className="grid" style={{ marginTop: 20 }}>
          <label style={{ display: "grid", gap: 7 }}><span style={{ fontSize: 13, color: "var(--muted)" }}>تاريخ التقرير</span><input type="date" value={reportDate} onChange={(e) => setReportDate(e.target.value)} /></label>
          <label className="upload wide" style={{ padding: 18, textAlign: "center" }}>اختر ملف Excel<input type="file" accept=".xlsx,.xls" onChange={handleFile} style={{ display: "none" }} /></label>
        </div>
